                    ...
                    <c:when test="${category eq null or category eq '전체'}">
                        <c:choose>
                            <c:when test="${search eq null or target eq null}">
                                <a class="" href="/index.jsp?page=${i}">${i}</a>
                            </c:when>
                            <c:otherwise>
            ...
            <c:forEach var="i" step="1" begin="0" end="${(count/8)}">
                <c:choose>
                    <c:when test="${category eq null or category eq '전체'}">
                        <c:choose>
                            <c:when test="${search eq null or target eq null}">
                                <a id="link" href="/index.jsp?page=${i+1}">${i+1}</a>
                            </c:when>
                            <c:otherwise>
                                <a id="link" href="/index.jsp?page=${i+1}&search=${search}&target=${target}}">${i+1}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:when>
                    <c:otherwise>
                        <c:choose>
                            <c:when test="${search eq null or target eq null}">
                                <a id="link" href="/index.jsp?page=${i+1}&category=${category}">${i+1}</a>
                            </c:when>
                            <c:otherwise>
                                <a id="link" href="/index.jsp?page=${i+1}&category=${category}&search=${search}&target=${target}">${i+1}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:otherwise>
                </c:choose>
